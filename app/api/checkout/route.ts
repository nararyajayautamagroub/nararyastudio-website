import { NextResponse } from "next/server";
import { getSessionUser } from "@/lib/auth";
import { db } from "@/lib/db";
import { orderId, safeText, isValidEmail } from "@/lib/security";
import { rateLimit } from "@/lib/rate-limit";
import { calculateCouponDiscount } from "@/lib/coupons";

export async function POST(req: Request) {
  const ip = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "unknown";
  if (!rateLimit("checkout:" + ip, 12, 10 * 60_000).allowed) {
    return NextResponse.json({ error: "Terlalu banyak checkout. Coba lagi nanti." }, { status: 429 });
  }

  try {
    const body = await req.json();
    const sessionUser = await getSessionUser();
    const requestedCustomerId = safeText(body.customerId, 80);
    const name = safeText(body.name, 120);
    const email = safeText(body.email, 160).toLowerCase();
    const couponCode = safeText(body.couponCode, 64).toUpperCase();

    if (!Array.isArray(body.items) || body.items.length === 0 || body.items.length > 50) {
      return NextResponse.json({ error: "Keranjang tidak valid." }, { status: 400 });
    }

    let customer = sessionUser;
    if (!customer && requestedCustomerId) {
      return NextResponse.json({ error: "Customer ID tidak boleh dikirim dari sesi guest." }, { status: 403 });
    }

    if (!customer) {
      if (!name || !isValidEmail(email)) {
        return NextResponse.json({ error: "Nama dan email valid wajib diisi." }, { status: 400 });
      }
      customer = await db.user.findUnique({ where: { email } });
      if (customer) {
        return NextResponse.json({ error: "Email sudah terdaftar. Login terlebih dahulu untuk checkout dengan akun ini." }, { status: 409 });
      }
      const base = (email.split("@")[0].replace(/[^a-z0-9_]/g, "").slice(0, 32) || "customer");
      let username = base;
      for (let n = 1; await db.user.findUnique({ where: { username } }); n++) {
        username = base + n;
      }
      customer = await db.user.create({ data: { name, username, email, passwordHash: "REQUEST_ONLY" } });
    }

    if (sessionUser && requestedCustomerId && requestedCustomerId !== sessionUser.id) {
      return NextResponse.json({ error: "Customer tidak sesuai session." }, { status: 403 });
    }

    const ids = body.items
      .map((x: unknown) => safeText((x as { productId?: unknown })?.productId ?? (x as { id?: unknown })?.id, 80))
      .filter(Boolean);
    const uniqueIds = [...new Set(ids)];
    const products = await db.product.findMany({
      where: { productId: { in: uniqueIds }, status: "PUBLISHED" },
    });
    const productMap = new Map(products.map((p) => [p.productId, p]));

    if (products.length !== uniqueIds.length) {
      return NextResponse.json({ error: "Ada produk yang tidak tersedia." }, { status: 400 });
    }

    let subtotal = 0;
    const items: { productId: string; quantity: number; price: number; discount: number }[] = [];

    for (const raw of body.items) {
      const key = safeText((raw as { productId?: unknown })?.productId ?? (raw as { id?: unknown })?.id, 80);
      const product = productMap.get(key);
      const quantity = Number((raw as { quantity?: unknown })?.quantity);
      if (!product || !Number.isInteger(quantity) || quantity < 1 || quantity > 99) {
        return NextResponse.json({ error: "Item checkout tidak valid." }, { status: 400 });
      }
      const discount = Math.min(100, Math.max(0, product.discount));
      const price = Math.max(0, product.price - Math.round((product.price * discount) / 100));
      subtotal += price * quantity;
      items.push({ productId: product.productId, quantity, price, discount });
    }

    const result = await db.$transaction(async (tx) => {
      let coupon: Awaited<ReturnType<typeof tx.coupon.findUnique>> = null;
      let couponDiscount = 0;

      if (couponCode) {
        coupon = await tx.coupon.findUnique({ where: { code: couponCode } });
        if (!coupon) throw new Error("COUPON_INVALID");
        couponDiscount = calculateCouponDiscount(coupon, subtotal);
        if (couponDiscount <= 0) throw new Error("COUPON_INVALID");
        const updated = await tx.coupon.updateMany({
          where: {
            id: coupon.id,
            active: true,
            ...(coupon.maxUses !== null ? { usedCount: { lt: coupon.maxUses } } : {}),
          },
          data: { usedCount: { increment: 1 } },
        });
        if (updated.count !== 1) throw new Error("COUPON_EXPIRED");
      }

      const total = Math.max(0, subtotal - couponDiscount);
      const order = await tx.order.create({
        data: {
          orderId: orderId(),
          customerId: customer.id,
          couponId: coupon?.id ?? null,
          subtotal,
          discount: couponDiscount,
          total,
          items: { create: items },
        },
      });

      await tx.notification.create({
        data: {
          userId: customer.id,
          title: "Order berhasil dibuat",
          message: "Order " + order.orderId + " dibuat dan menunggu pembayaran.",
          type: "ORDER",
        },
      });
      return order;
    });

    return NextResponse.json({
      orderId: result.orderId,
      total: result.total,
      discount: result.discount,
      status: result.status,
      paymentStatus: result.paymentStatus,
    }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "";
    if (message === "COUPON_INVALID" || message === "COUPON_EXPIRED") {
      return NextResponse.json({
        error: "Coupon tidak valid, belum aktif, sudah berakhir, minimum belanja belum tercapai, atau kuotanya habis.",
      }, { status: 400 });
    }
    return NextResponse.json({ error: "Checkout gagal." }, { status: 500 });
  }
}