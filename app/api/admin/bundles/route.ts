...
export async function POST(req:Request){
  const staff=await requireStaff(ROLES);if(!staff)return NextResponse.json({error:"Forbidden"},{status:403});
  try{
    const b=await req.json();const name=safeText(b.name,160),description=safeText(b.description,5000),discountPercent=Number(b.discountPercent??0);const products:unknown[]=Array.isArray(b.products)?b.products:[];
    ...
    const productIds:string[]=products.reduce<string[]>((acc,p)=>{const id=safeText(p,80);if(id&&!acc.includes(id))acc.push(id);return acc},[]);
